...
import http.cookiejar, urllib.request, urllib.error, urllib.parse, urllib.request, urllib.parse, urllib.error, sys, re, os, webbrowser, time, unicodedata, logging
...
class LegendasTV(SubtitleDatabase.SubtitleDB):
    ...
    def LegendasTVLogin(self):
        ...
        login_data = urllib.parse.urlencode({'txtLogin':username,'txtSenha':password})
        request = urllib.request.Request(self.url+'/login_verificar.php',login_data)
    ...
    def LegendasTVMovies(self, file_original_path, title, year, langs):
        ...
        search_data = urllib.parse.urlencode(search_dict)
        request = urllib.request.Request(self.url+'/index.php?opcao=buscarlegenda',search_data)
        response = self.to_unicode_or_bust(urllib.request.urlopen(request).read())
        ...
        for x in range(pages):
            if x:
                html = urllib.request.urlopen(self.url+'/index.php?opcao=buscarlegenda&pagina='+str(x+1)).read()
